...
.macro SubBytes8 reg
    ...
    sub.16b v13, v12, v11
    tbx.16b \reg, {v20-v23}, v12
    sub.16b v14, v13, v11
    tbx.16b \reg, {v24-v27}, v13
    tbx.16b \reg, {v28-v31}, v14
.endm
...
.macro MixColumns8 reg
    mov.16b v12, \reg               // save original s

    tbl.16b v13, {v12}, v9          // rot1 = s[(j+1)%4]
    eor.16b v14, v12, v13           // v14 = s ^ rot1

    // Compute t = s ^ rot1 ^ rot2 ^ rot3
    tbl.16b v15, {v13}, v9          // rot2
    eor.16b v13, v14, v15           // s ^ rot1 ^ rot2
    tbl.16b v15, {v15}, v9          // rot3
    eor.16b v13, v13, v15           // t = s ^ rot1 ^ rot2 ^ rot3

    // xtime(s ^ rot1) = xtime(v14)
    sshr.16b v15, v14, #7           // 0x00 or 0xFF
    shl.16b v14, v14, #1
    and.16b v15, v15, v10           // 0x00 or 0x1b
    eor.16b v14, v14, v15           // xtime(s ^ rot1)

    // result = s ^ t ^ xtime(s ^ rot1)
    eor.16b \reg, v12, v13          // s ^ t
    eor.16b \reg, \reg, v14         // s ^ t ^ xtime
.endm
...
_AES_Parallel_8PT:
    ...
    adrp x4, aes_sbox_data_8pt@PAGE
    add x4, x4, aes_sbox_data_8pt@PAGEOFF
    ...
    adrp x4, shiftrows_idx_8pt@PAGE
    add x4, x4, shiftrows_idx_8pt@PAGEOFF
    ld1.16b {v8}, [x4], #16
    ld1.16b {v9}, [x4]
