...
.macro SubBytes4 reg
    sub     v12.16b, \reg\().16b, v11.16b       // idx - 0x40
    tbl     \reg\().16b, {v16.16b, v17.16b, v18.16b, v19.16b}, \reg\().16b   // [0-63]
    sub     v13.16b, v12.16b, v11.16b           // idx - 0x80
    tbx     \reg\().16b, {v20.16b, v21.16b, v22.16b, v23.16b}, v12.16b       // [64-127]
    sub     v14.16b, v13.16b, v11.16b           // idx - 0xC0
    tbx     \reg\().16b, {v24.16b, v25.16b, v26.16b, v27.16b}, v13.16b       // [128-191]
    tbx     \reg\().16b, {v28.16b, v29.16b, v30.16b, v31.16b}, v14.16b       // [192-255]
.endm
...
.macro MixColumns4 reg
    mov     v4.16b, \reg\().16b                 // save original

    tbl     v5.16b, {v4.16b}, v9.16b            // rot1: s[(j+1)%4]
    tbl     v6.16b, {v5.16b}, v9.16b            // rot2
    tbl     v7.16b, {v6.16b}, v9.16b            // rot3

    eor     v12.16b, v4.16b, v5.16b             // s ^ rot1
    eor     v13.16b, v6.16b, v7.16b             // rot2 ^ rot3
    eor     v14.16b, v12.16b, v13.16b           // t

    // xtime(s ^ rot1)
    sshr    v15.16b, v12.16b, #7                // 0x00 or 0xFF
    shl     v12.16b, v12.16b, #1
    and     v15.16b, v15.16b, v10.16b           // 0x00 or 0x1b
    eor     v12.16b, v12.16b, v15.16b           // xtime result

    eor     \reg\().16b, v4.16b, v14.16b        // s ^ t
    eor     \reg\().16b, \reg\().16b, v12.16b   // s ^ t ^ xtime
.endm
...
_AES_Parallel_4PT:
    ...
    adrp    x4, aes_sbox_data
    add     x4, x4, :lo12:aes_sbox_data
    ...
    adrp    x4, shiftrows_idx
    add     x4, x4, :lo12:shiftrows_idx
    ld1     {v8.16b}, [x4], #16
    ld1     {v9.16b}, [x4]              // rot1_idx
